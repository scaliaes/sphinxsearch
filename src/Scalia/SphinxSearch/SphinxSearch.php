...
class SphinxSearch
{
    ...
    public function get($respect_sort_order = FALSE)
    {
        ...
        if($result)
        {
            ...
            // Get total hits for search.
            $this->_total_hits = $result['words'][$this->_search_string]['hits'];
            ...
            if($result['total'] > 0 && isset($result['matches']))
            {
                ...
                if ($this->_mapping)
                {
                    ...
                    {
                        $result = $this->queryDatabase('model',$matchids);
                    }
                    else
                    {
                        $result = $this->queryDatabase('table',$matchids);
                    }
